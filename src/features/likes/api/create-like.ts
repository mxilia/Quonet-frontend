...
export const useCreateLike = ({ mutationConfig }: useCreateLikeOptions = {}) => {
  ...
  return useMutation({
    ...
    onError: (_err, _vars, ctx: any) => {
      if (!ctx) return
      if (ctx.previousLikeState)
        queryClient.setQueryData(ctx.likeStateQueryKey, ctx.previousLikeState)
      if (ctx.previousLikeCount)
        queryClient.setQueryData(ctx.likeCountQueryKey, ctx.previousLikeCount)
    },
