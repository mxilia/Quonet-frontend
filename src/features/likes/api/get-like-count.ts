...
const likeCountQueryKey = ({ parentType, ownerId, parentId } : { parentType ?: string, ownerId ?: string, parentId ?: string }) => {
...
export const getLikeCountQueryOptions = (parentType ?: string, ownerId ?: string, parentId ?: string) => {
  return queryOptions({
    ...
    queryFn: () => {
      return getLikeCount({ parentType: parentType ?? "", ownerId: ownerId ?? "", parentId: parentId ?? "" });
    },
  })
}
